...
As of 2026-07-09, solver development follows stages S0-S6 defined in `docs/strategy/solver-design.md` (design) and `docs/strategy/solver-implementation-plan.md` (implementation spec). M0 and M1 below remain active contracts: the measurement backbone and the submission-safety shell, both of which the new solver keeps. The M2-M4 sections are historical scope records superseded by the S-track, and M5 corresponds to stage S6 hardening.
...
Status: rescoped 2026-07-09 to the fast-constructor track; execution detail lives in `docs/strategy/m2-m4-execution-plan.md`. Adaptive block-order selection remains the last accepted M2 result.
...
As of 2026-07-09 the unfinished queue items above are folded into the fast-constructor slices in `docs/strategy/m2-m4-execution-plan.md` (time candidates and placement candidates into M2-B, left-shift polish into M2-C, single-block reinsert into the M3 operator roadmap); do not run them as standalone branches.
...
Status: integrated but measured inactive at 60s. The 2026-07-09 diagnostic showed LNS entered on 1/40 daily-40 instances with 0 accepted candidates, and the earlier dev-10 delta traces to baseline run variance, not LNS. M3 execution is now gated on the M2 fast constructor; see `docs/strategy/m2-m4-execution-plan.md`.
...
Status: profiling gated; execution detail in `docs/strategy/m2-m4-execution-plan.md`. M4a profiling runs after the M2 fast constructor lands, because the constructor rewrite changes the hotspot map.
...
1. Execute the clean-slate solver design in `docs/strategy/solver-design.md` using the implementation spec in `docs/strategy/solver-implementation-plan.md` (module layout, ALNS operators, SA acceptance, CP-SAT models, tests). Start with tasks S0-1..S0-3 (contract tests, kernel skeleton, microbenchmarks), then S1. This supersedes the M2 fast-constructor track in `docs/strategy/m2-m4-execution-plan.md`.
2. Treat the 2026-07-09 diagnostic (`experiments/results/m3/lns_diagnostic/2026-07-09-m3-lns-diagnostic-dev-10-myalgorithm-300s-small.json`) and plan §1 as the authoritative diagnosis: Phase-1 greedy consumes any budget and LNS is effectively dead at 60s.
3. Keep the accepted M3 artifacts as the active comparison baseline numbers (dev-10 total `3620223014.45645`, daily-40 total `25376460025.16775`) until a fast-constructor result replaces them.
4. Apply the measurement-integrity rules in plan §6.3 (structural evidence, `--lns-mode off` A/B attribution, rerun-noise band) to every improvement claim.
5. Continue one experiment branch at a time; do not mix multiple hypotheses in one merge.
6. Keep full Fable ALNS, CP-SAT retiming, and compiled/raster geometry acceleration deferred until separately justified (plan §4.5, §5.3).
7. Use `--solver myalgorithm` when measuring the submission entry point, and

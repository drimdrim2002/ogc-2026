...
Scope note (2026-07-09): the branch, result-path, accept-criteria, and decision-state conventions in this playbook also govern the S-track (stages S0-S6 from `docs/strategy/solver-design.md`). For S-track work use branch names `codex/s<stage>-<slice>` and result paths `experiments/results/s<stage>/<slice>/...`; everything else applies unchanged. The M2 queue table below is historical (folded into the S-track).

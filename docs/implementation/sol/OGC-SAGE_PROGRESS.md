...
- 다음 단계 진입: constructor/feature gate까지 통과해 final matrix 진입 가능. D-015에 따라 heuristic `LNS_ENABLED=True`, candidate MIP default disabled, `INTERLOCK_ENABLED=False`를 유지한다. 9단계 완료 또는 release 가능을 아직 의미하지 않는다.
...
| D-015 | 2026-07-13 | 9D-1 | P5/P6 default 결정 | source `ba5aa73`, dense 9×60초×2 seeds에서 heuristic LNS는 constructor+retime 대비 18W/0T/0L이고 6개 operator 모두 attempts/feasible/accepted>0이므로 P5 heuristic-only를 ON한다. candidate MIP은 heuristic 대비 9W/1T/8L로 worst regression이 있어 default OFF다. interlock variant는 18/18 Stage 5지만 densify trigger/attempt/improvement가 모두 0이므로 strict-improvement 증거가 없어 P6 OFF다. 모든 variant에서 exception/outer-timeout/feasibility regression은 0이다. | `baseline/solver/{entry,runtime}.py`, activation tests, feature raw/summary/comparison | final 600은 exact `heuristic_lns` default commit에서만 실행; threshold/subset 변경 없음 |
...
| P5/P6 enable gate | §13 | 6~9 | constructor gate PASS 후 dense 9×60초×2 seeds 비교 | D-015: heuristic LNS ON, candidate MIP OFF, interlock OFF. P5 18W/0T/0L 및 operator activity 충족; MIP 8 loss, P6 densify activity 0으로 미활성화 |
